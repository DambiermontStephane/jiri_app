<x-layouts.app>
    <section class="flex justify-center items-center h-screen">
        <div class="shadow-2xl p-5 rounded-2xl border-2 border-gray-200">
            ...
            <h1 class="text-center text-2xl font-bold mb-5">Connexion à l'espace Jury</h1>
            <form action="{{ route('login.store') }}" method="post" class="flex flex-col items-center">
                @csrf
                <div class="flex content-center flex-col gap-5 items-center w-80">
                    <x-form.label-input name="email" type="text" label="Adresse e-mail"></x-form.label-input>
                    ...
                    <button type="submit" class="text-white bg-blue-500 rounded-sm p-2 w-full">Se connecter</button>
                    <p>Pas encore de compte ? <a href="{{ route('register.store') }}" class="text-blue-500">Créer un
                            compte</a></p>
                </div>
            </form>
        </div>
